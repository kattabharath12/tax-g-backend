...
async function processDocument(document, onProgress) {
    try {
        ...
        if (process.env.GOOGLE_CLOUD_PROJECT_ID && process.env.GOOGLE_APPLICATION_CREDENTIALS) {
            try {
                ...
                logger_1.logger.warn('Google Document AI failed, falling back to LLM processing:', error);
            }
        }
...
async function processWithGoogleDocumentAI(document, onProgress) {
    ...
    // Read the file
    const fileBuffer = await promises_1.default.readFile(document.filePath);
...
async function processWithLLM(document, onProgress) {
...
function extractStructuredData(processedDoc, documentType) {
...
function convertToTaxFormat(extractedData, documentType) {
    const taxData = {
        documentType,
